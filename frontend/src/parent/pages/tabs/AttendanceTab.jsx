...
import attendanceService from "@/parent/services/attendance";
import { getAllSessions } from "../../../subadmin/services/classService";

export default function AttendanceTab({ child, classSubjects }) {
  ...
  const loadAttendance = async (class_subject_id, date) => {
    if (!class_subject_id || !date || !child?.id) return;
    setLoading(true);
    setErr(null);
    try {
  ...
  useEffect(() => {
    if (selectedClassSubject?.class_subject_id && selectedDate && child?.id) {
      loadAttendance(selectedClassSubject.class_subject_id, selectedDate);
    } else {
      setItems([]);
    }
  }, [selectedClassSubject, selectedDate, child?.id]);
      ...
      <CardContent>
        ...
        {sessionLoading ? (
          ...
          <Alert severity="warning">
            {!child ? "No child information found." : "No current session found. Please contact administrator."}
          </Alert>
        )}
      </CardContent>
    </Card>
