  ...
  return (
    <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={2} className="mb-4 flex-wrap gap-4">
      <Box className="min-w-0">
        <Typography variant="h6" fontWeight={900} className="text-[1rem] leading-tight md:text-[1.05rem]">
          {title}
        </Typography>
        {subtitle ? (
...
export function Panel({ children, sx }) {
  return (
    <Card
      elevation={0}
      className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-[0_18px_50px_rgba(2,6,23,0.28)] backdrop-blur-xl"
      sx={sx}
    >
