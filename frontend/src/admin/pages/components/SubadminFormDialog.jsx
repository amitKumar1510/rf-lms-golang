  ...
  return (
    ...
    >
      ...
      <DialogContent className="pt-6">
        <Box className="pt-1">
          {err ? (
            <Alert severity="error" className="mb-4 rounded-xl border border-red-400/20 bg-red-950/40">
              {String(err)}
            </Alert>
          ) : null}
      ...
      <DialogActions sx={{ px: 3, pb: 2.5, pt: 1.5, borderTop: "1px solid rgba(148, 163, 184, 0.12)" }}>
        <Button onClick={onClose} color="inherit" className="!rounded-lg !px-4 !py-2 !text-sm">
          Cancel
        </Button>
        <Button onClick={handleSubmit} variant="contained" disabled={!canSubmit || saving} className="!rounded-lg !px-4 !py-2 !text-sm !shadow-none">
